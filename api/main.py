...
def _document_tool_def() -> dict:
    ...
    return {
        ...
        "description": (
            ...
            "и мягкая коррекция освещения. Замена фона — отдельный параметр, по "
            "умолчанию выключенный. Пользователь сразу "
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "light": {
                    "type": "number",
                    "description": (
                        "Выравнивание освещения лица, 0..0.8. Не передавай при первом вызове — "
                        "сила подберётся по замеру неравномерности света. При правке отталкивайся "
                        "от значения в текущем черновике: усилить = взять БОЛЬШЕ, а не «до максимума». "
                        "В отчёте есть light_unevenness (остаточная неравномерность) и "
                        "light_unevenness_before — по ним видно, помогла правка или нет."
                    ),
                },
                "fix_head_tilt": {
                    "type": "boolean",
                ...
                "remove_background": {
                    "type": "boolean",
                    "description": (
                        "Заменить фон на белый. По умолчанию false — фон остаётся как на "
                        "исходном фото. Ставь true ТОЛЬКО если пользователь прямо попросил "
                        "убрать/заменить/отбелить фон, либо если ты предложил это из-за "
                        "плохого фона и пользователь согласился. Сегментация срезает тонкие "
                        "пряди волос, поэтому на нормальном светлом фоне её включать не надо."
                    ),
                },
            },
            "required": ["profile"],
...
async def ai_photo_chat(req: PhotoChatRequest, request: Request):
    ...
    if doc_allowed:
        doc_block = (
            ...
            "- Фон НЕ заменяется сам собой. Если по отчёту фон плохой (тёмный, пёстрый, "
            "с тенями) — скажи об этом и предложи заменить, но включай remove_background "
            "только после явной просьбы. Предупреждай, что при замене могут обрезаться "
            "тонкие пряди волос.\n"

...
### Document Photo Tool (Sep 2, 2026)

- **Goal**: prepare a passport-style photo from an archive photo, driven from the AI chat in the lightbox. Poland only for now (karta pobytu, MOS upload + 35x45 mm print), built to extend.
- **Specs live as data** in `config/document_photo_specs.py` — adding a country/document is adding a dict entry, no code change. Every number carries `source` (where, when, how trusted) so the assistant can say "это со вторичных источников", and `calibration` recording what was measured against a real accepted output.
- **Geometry is anchored on the MOS editor's own two guides**, `Czubek glowy` at 11.0% from the top and `Podbrodek` at 87.5% (5.6 mm from the bottom), 76.5% = 34.4 mm between them — measured off editor screenshots, ~0.5 mm precision. Head is therefore crown-to-chin: the crown from the u2net mask (hair included, restricted to a band around the face so a raised hand cannot pass for the crown), the chin from the bbox. Two failures got us here: measuring the head with the bbox alone leaves the hair out and gives 37 mm heads (norm 32-36) with 1.8 mm of clearance; and anchoring the frame vertically on the eye line (58.7%, calibrated from one accepted output) put the whole head ~2 mm above BOTH guides when loaded back into the editor. Anchoring on the crown pins both at once — verified 4.84 mm vs their 4.95 mm. Where the eye lands is now reported, not enforced: no checked source for a Polish 35x45 eye-line tolerance was found, and an invented one only fired false warnings. Horizontal centring is nose-weighted (`0.7*nose + 0.3*eye_mid`) — on a slightly turned head the bbox centre drifts off the face axis.
- **Segmentation**: `u2net.onnx` run directly on the already-installed `onnxruntime`, mounted at `/models` (volume `doc_models`; the file was copied in to skip a 176 MB download). **`rembg` must not be added** — it pulls pymatting→numba→numpy 2.x against the project's `numpy<2` pin. Mask is clamped `(m-0.02)/0.96` so the background composites to exactly 255 and the halo shrinks.
- **Background replacement is opt-in** (`remove_background`, default false; the assistant sets it only on an explicit request). The mask is always computed — the crown is read off it — but applying it to the pixels shaves thin hair strands, so on an already-even background it is a downgrade. The report therefore measures two independent things: `background_whiteness` (how close to the required colour) and `background_spread`/`background_even` (shadows, gradient). Conflating them labelled a perfectly even beige wall "неоднородный", which is simply false; now the assistant can say "ровный, но не белый" and offer the swap with its cost.
- **Corrections are tone-only** (no retouch, no CLAHE — the specs forbid it and a fixed aggressive setting looks unnatural): illumination flattening, contrast, brightness, white balance. White balance is **white-patch estimated outside the face** — grey-world on the face literally greys out skin, which is not neutral. `light` defaults to `clip(unevenness*1.5, 0, 0.6)`.
- **Order of operations is a 100x performance fact**: crop and downscale to the output size FIRST, correct after. The illumination blur uses a sigma proportional to the face, so on a full-resolution 12 MP original that is a ~1000px-sigma convolution — 117 s per render. On the 684x883 crop the whole render is ~1 s (6.6 s cold including load/detect/segment). The field is low-frequency, so nothing is lost.
- **Delivery**: the preview is returned as a URL and deliberately kept OUT of `conversation_history` — re-sending a ~1500-token image every "усиль контраст" turn costs more than it is worth. The trade-off is explicit: the user judges the picture, the model steers by parameters and the numeric report. The last draft's parameters are restated in the system prompt so edits are relative. Detection + segmentation are cached per file (~30 min), so an edit turn only redoes the cheap part.
- **Two tools** in `/ai/photo-chat`: `prepare_document_photo` (profile + 4 adjustments + tilt fix) and `finalize_document_photo` (`save` | `download`). The finalize tool exists because "сохрани"/"скачай" must work as words — an earlier version only had buttons and the assistant kept telling the user to press a button that a stale cached JS had not rendered. A download cannot be started server-side, so it comes back as `action: {type: "download", url}` and the browser follows it.
- **Save** writes to `_DOCUMENTS/{user_id}/{YYYY-MM-DD}/`, then indexes CLIP and faces **inline** (not waiting for a batch) and tags "документы". Use `get_face_indexer()`, never the `face_indexer` global — on a fresh container the global is still None and faces get silently skipped.
- **Static files now revalidate** (`RevalidatingStaticFiles` sets `Cache-Control: no-cache`). The shared JS components are edited in place through a bind mount with no build step and no hashed filenames; plain `StaticFiles` sends no Cache-Control, so browsers cached `photo_ai_chat.js` heuristically and served a stale copy long after deploy — which is what hid the preview card.
- **Report fields must be coerced with `float()`/`bool()`** — they derive from numpy scalars, and a single `np.float32`/`np.bool_` makes the whole `/ai/photo-chat` response unserialisable.
- **RBAC**: section `documents`, assignable function `documents.create` (`sql/migrate_add_documents.sql`), granted to all existing users. Tunnel access allowed.
- **Not implemented**: head-contour validation (cut hair / cut ear / halo are official rejection reasons), glasses / expression / gaze checks are left to the vision model in prose.

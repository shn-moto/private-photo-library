...
- **Geometry — head is crown-to-chin, not the detector bbox.** This is the dimension the regulation constrains and the boundary the MOS editor makes you drag. The crown comes from the u2net mask (hair included, restricted to a band around the face so a raised hand cannot pass for the crown), the chin from the bbox. Measuring with the bbox alone left the hair out and produced 37 mm heads (norm 32-36) with 1.8 mm above the crown. With the fix: 34.0 mm, 3.3 mm top margin. Eye line at 58.7% from the bottom is the one figure calibrated from a real mosfoto.com output. Horizontal centring is nose-weighted (`0.7*nose + 0.3*eye_mid`) — on a slightly turned head the bbox centre drifts off the face axis.

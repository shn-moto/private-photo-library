...
- **Родовое дерево (🌳)** — интерактивная визуализация семейных связей (Graphviz layout); связи родитель/супруг; цветовые ветки; pan/zoom; дата рождения; фокус на конкретную персону
...
- **AI помощник (Gemini)** — умный поиск на естественном языке ("найди Сашу в Камбодже", "закат на пляже"); двухуровневая географическая точность (город → GPS-рамка, конкретное место → GPS-рамка + визуальный CLIP-запрос); серверный геокодинг через Nominatim для точных координат
- **AI чат по фото (Gemini Vision)** — кнопка ✨ в lightbox открывает скользящую панель чата; Gemini Vision анализирует фото с полным контекстом (EXIF, GPS, дата, имена персон, теги); быстрые кнопки: "Что на фото?", "Где это?", "Что написано?", "Переведи текст"
...
- **Информация о фото (ℹ)** — EXIF-данные (GPS, камера, объектив, ISO, даты) в попапе по кнопке ℹ в lightbox; администратор может редактировать дату снимка и GPS координаты; бэйдж ℹ на миниатюрах
- **Зум и панорамирование** — scroll-зум, pinch-to-zoom, перетаскивание при увеличении; double-click → 1:1 / fit
- **Полный размер (🔍)** — загрузка оригинального изображения без ограничения 2000px для macroscale zoom
- **Полноэкранный режим (⛶)** — кнопка или F11 для fullscreen-просмотра
...
- **Telegram бот** — поиск фотографий прямо из Telegram; команды `/feed` (лента), `/map` (карта) и `/books` (библиотека) открывают Web UI через защищённый туннель
...
- **Родовое дерево (🌳)** — интерактивная визуализация семейных связей (Graphviz); связи родитель/супруг/со-родитель; цветовые ветки для каждой пары родителей; pan/zoom; фокус на конкретную персону
...
| `/image/{id}/original` | GET | Оригинальный размер (без лимита, для зума) |
...
| `/persons/{id}` | PUT | Обновить персону (name, description, birth_date) |
...
| `/persons/relations` | POST | Создать связь {from, to, type: parent/spouse} |
| `/persons/relations/{id}` | DELETE | Удалить связь |
| `/persons/family-tree` | GET | Полное дерево: персоны + связи |
| **Привязка лиц** | | |
...
| `/ai/photo-chat` | POST | AI Vision Q&A о фото (Gemini Vision + контекст EXIF/лица/теги) |
| **Admin** | | |
...
- **Lightbox с лицами:**
  ...
  - Кнопка ℹ — EXIF-информация (камера, объектив, ISO, GPS, даты); admin может редактировать дату и GPS
  - Кнопка 🔍 — загрузка оригинального размера (без ограничения 2000px)
  - Кнопка ⛶ / F11 — полноэкранный режим
  - Зум: скролл, pinch-to-zoom, +/-, double-click; панорамирование при увеличении
  - Кнопка ✨ — AI чат по фото (Gemini Vision): описание, локация, OCR, перевод, свободные вопросы
...
- `F11` — полноэкранный режим lightbox
- `+` / `-` — зум в lightbox
- `0` — сбросить зум (по размеру экрана)
...
│       ├── family_tree.html # Родовое дерево (Graphviz визуализация)
│       ├── library.html    # Библиотека книг (стилизованная книжная полка)
│       └── photo_ai_chat.js # AI Vision чат панель для lightbox (Gemini)
├── bot/
...
- ✅ **AI помощник (Gemini)** — умный поиск на естественном языке (chat-based, карта + поиск); двухуровневая точность мест (город=GPS, POI=GPS+CLIP); серверный геокодинг (Nominatim) для уточнения координат; мультиязычность (русский, польский, английский)
...
- ✅ **EXIF Info (ℹ)** — кнопка информации на lightbox и бэйдж на миниатюрах; EXIF, GPS, камера, объектив, ISO
- ✅ **Зум и панорамирование в lightbox** — scroll-зум, pinch-to-zoom, перетаскивание при увеличении, double-click 1:1
- ✅ **Полный размер (оригинал)** — кнопка 🔍 в lightbox загружает изображение в оригинальном размере (без ограничения 2000px)
- ✅ **Полноэкранный режим** — кнопка ⛶ или F11 в lightbox
- ✅ **AI чат по фото (Gemini Vision)** — кнопка ✨ в lightbox; Q&A с полным контекстом (EXIF, лица, теги, GPS); 7 режимов (описание, локация, OCR, перевод и др.)
- ✅ **Библиотека книг (library.html)** — разбиение HTML-книг на главы с оглавлением; pHash-сопоставление изображений с фотоархивом; ссылки на карту по image_id; стилизованная книжная полка; Telegram команда `/books`
- ✅ **Родовое дерево (family_tree.html)** — интерактивная визуализация семейных связей (Graphviz); связи родитель/супруг/со-родитель; цветовые ветки; pan/zoom; дата рождения; фокус на конкретную персону
...
- ✅ **Складные карточки в Admin UI** — сворачиваемые блоки на админке с сохранением состояния в localStorage
